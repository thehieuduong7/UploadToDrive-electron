		...
		(e) =>
			`   <a href="${e.webViewLink}" target="_blank"
                class="list-group-item
                    ...
                    link-item
                    ">
                    ${e.name} (${e.mimeType}) - size: ${bytesToMB(
				e.size
			)}MB - time: ${e.createdTime}
            </a>`
	);
...
$(".file-path").change(function () {
	validPath.bind(this)((path) => ipcRenderer.sendSync("check-file-path", path));
});
$(".directory-path").change(function (event) {
	validPath.bind(this)((path) => ipcRenderer.sendSync("check-path", path));
});

const form = $("#form-config");
$("#submit-config").click(function submitPath(event) {
	event.preventDefault();
